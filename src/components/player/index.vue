  ...
  >
    ...
    <div class="w-24 whitespace-nowrap overflow-hidden">
      <p
        ...
        :style="{
          animationDuration: musicName && musicName.offsetWidth / 30 + 's',
        }"
        ref="musicName"
        :text="music.name"
      >
      ...
      <P
        class="text-sm font-bold words"
        :class="musicAuthor && musicAuthor.offsetWidth > 96 && 'move-words'"
        :style="{
          animationDuration: musicAuthor && musicAuthor.offsetWidth / 30 + 's',
        }"
        ref="musicAuthor"
        :text="music.author"
        >{{ music.author }}</P
    ...
    <div
      class="ml-5 cursor-pointer transition duration-300 hover:scale-110"
      @click="handlePlayMusicClick"
    >
      <svg
      ...
      >
        <path
          d="M515.584 64.512c-247.808 0-449.024 201.216-449.024 449.024S267.776 962.56 515.584 962.56s449.024-201.216 449.024-449.024S763.392 64.512 515.584 64.512z m0 834.048c-211.968 0-384.512-172.544-384.512-384.512s172.544-384.512 384.512-384.512S900.096 302.08 900.096 514.048 727.552 898.56 515.584 898.56z"
          p-id="1421"
          fill="#111"
        ></path>
        <path
          d="M373.76 660.992c0 40.96 28.672 57.344 64 36.864l254.464-146.944c35.328-20.48 35.328-53.76 0-74.24L437.76 329.728c-35.328-20.48-64-3.584-64 36.864v294.4z"
          p-id="1422"
          fill="#111"
        ></path>
      </svg>
      ...
      >
        <path
          d="M511.445333 64c-217.429333 0-408.234667 158.549333-442.197333 380.074667-37.546667 244.522667 130.304 473.173333 374.826667 510.72 23.04 3.498667 45.909333 5.205333 68.48 5.205333 217.429333 0 408.234667-158.549333 442.197333-380.074667 37.546667-244.522667-130.304-473.173333-374.826667-510.72a451.584 451.584 0 0 0-68.48-5.205333m0 64c19.498667 0 39.253333 1.536 58.752 4.48a382.72 382.72 0 0 1 251.306667 152.32 382.72 382.72 0 0 1 69.973333 285.44 382.464 382.464 0 0 1-130.56 234.026667 383.829333 383.829333 0 0 1-558.421333-65.066667 382.72 382.72 0 0 1-69.973333-285.44A382.464 382.464 0 0 1 263.04 219.733333 383.829333 383.829333 0 0 1 511.445333 128"
          p-id="4179"
          fill="#111"
        ></path>
        <path
          d="M608 352a32 32 0 0 0-32 32v256a32 32 0 0 0 64 0V384a32 32 0 0 0-32-32M416 352A32 32 0 0 0 384 384v256a32 32 0 0 0 64 0V384a32 32 0 0 0-32-32"
          p-id="4180"
          fill="#111"
        ></path>
      </svg>
  ...
  setup() {
    const state = reactive({
      ...
      musicList: [
        ...
        { name: `伝記`, author: '上野耕路', url: 'src/assets/2.mp3' },
        { name: '公園', author: '上野耕路', url: 'src/assets/3.mp3' },
      ] as music[],
      isUp: false,
    })
    ...
    const actions = {
      handlePlayMusicClick() {
        state.music.isPlaying
          ? (player.value as HTMLAudioElement).pause()
          : (player.value as HTMLAudioElement).play()
        state.music.isPlaying = !state.music.isPlaying
      },
      handleTimeUpdate() {
        state.music.currentTime = Number(
          (player.value as HTMLAudioElement).currentTime.toFixed(2)
        )
        state.music.duration = Number(
          (player.value as HTMLAudioElement).duration.toFixed(2)
        )
      },
